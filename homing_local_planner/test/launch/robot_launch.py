...
def generate_launch_description():
    package_dir = get_package_share_directory('homing_local_planner')
    ...
    webots = WebotsLauncher(
        world=PathJoinSubstitution([package_dir, 'test/worlds', world]),
        mode=mode,
    ...
    robot_description_path = os.path.join(package_dir, 'test/resource', 'turtlebot_webots.urdf')
    ros2_control_params = os.path.join(package_dir, 'test/resource', 'ros2control.yml')
    ...
    nav2_map = os.path.join(package_dir, 'test/resource', 'turtlebot3_burger_example_map.yaml')
    nav2_params = os.path.join(package_dir, 'test/resource', 'nav2_params.yaml')
    if 'turtlebot3_navigation2' in get_packages_with_prefixes():
        turtlebot_navigation = IncludeLaunchDescription(
            PythonLaunchDescriptionSource(os.path.join(package_dir, 'test/launch', 'navigation2.launch.py')),
            launch_arguments=[
                ('map', nav2_map),
